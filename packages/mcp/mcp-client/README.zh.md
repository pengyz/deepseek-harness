...
每个 MCP 工具都有两个名称：通过 `tools/call` 在协议上传送的原始 MCP 名称，以及公开名称 `mcp__<serverName>__<rawName>`，后者注册到 `ctx.tools`。公开名称会规范化为本 harness 所对接的全部协议的交集（64 个字符、`[A-Za-z0-9_]`，不含连字符——DeepSeek 自身的约定允许结尾连字符，但 Claude 的 Messages API 工具名校验会拒绝连字符）；如果替换或截断改变名称，就会追加 `(serverName, rawName)` 的确定性 12 位十六进制 hash，确保不同工具绝不会折叠为同一个名称。名称是 `(serverName, rawName)` 的纯函数：连接顺序、重新同步和其他服务器永远不会重命名工具。
